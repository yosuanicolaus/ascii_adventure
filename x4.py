...
def is_inside_circle(y, x):
    distance = math.sqrt((abs(CENTERX - x) ** 2) + (abs(CENTERY - y) ** 2))
    return distance <= RADIUS


def shift_z_axis(y, x):
    ratio_to_center = 1 - math.sqrt((abs(CENTERX - x) ** 2) +
                                    (abs(CENTERY - y) ** 2)) / RADIUS
    # ratio_to_center = abs(CENTERY - y) / RADIUS
    new_y = y - int(ratio_to_center * Z_COEFFICIENT)
    return new_y
...
def rotate_points(angle):
    for idx in range(len(POINTS)):
        ...
        if is_inside_circle(new_y, new_x):
            new_y = shift_z_axis(new_y, new_x)
            ICON = ';'

        if 0 <= new_x < PLANE_LENGTH_X and 0 <= new_y < PLANE_WIDTH_Y:
            CANVAS_Y_X[new_y][new_x] = ICON
